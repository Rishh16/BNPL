 ...
 * }
 */
router.post("/create-group", requireLogin, requireKycApproved, requireRiskAllowed, (req, res) => {
  const userId = req.session.user_id;
...
// ============================================================
router.post("/join-group", requireLogin, requireKycApproved, requireRiskAllowed, (req, res) => {
  const userId = req.session.user_id;
...
router.post("/pay-installment", requireLogin, (req, res) => {
  ...
  db.query(
    ...
    (err, installments) => {
      ...
      db.query(
        "UPDATE group_installments SET status='PAID', paid_amount=amount, payment_date=CURDATE() WHERE id=?",
        [installmentId],
        (err2) => {
          if (err2) return res.status(500).json({ error: err2.message });
          ...
          if (today <= dueDate) {
            db.query(
              `INSERT INTO group_incentives 
               ...
               VALUES (?, ?, 'ON_TIME_BONUS', ?, 'EARNED', DATE_ADD(CURDATE(), INTERVAL 30 DAY))`,
